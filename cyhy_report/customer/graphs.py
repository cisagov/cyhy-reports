...
class MyStackedBar(object):
    ...
    def plot(self, filename, size=1.0):
        ...
        try:
            leg = plt.legend(bars, self.dataLabels, ncol=len(self.dataLabels), loc='upper center', fancybox=True, prop={'size':4})
    ...
    def plot(self, filename, size=1.0):
        ...
        if self.barSeverities:
            ...
            if self.legendLabels:                           # build a dummy set of bars ('underneath' the real bars) to be used 
                ...
                dummy_legend_rects = plt.bar(pos, self.series.values, align='center', color=legendColors, edgecolor='white', width=0.5)
                leg = plt.legend(dummy_legend_rects, self.legendLabels, ncol=len(self.legendLabels), loc='upper center', fancybox=True, prop={'size':4}, bbox_to_anchor=(0.5, 1.2))
                leg.get_frame().set_alpha(0.5)
...
class MyDistributionBar(object):
    ...
    def plot(self, filename, size=1.0):
        ...
        for (day,bgcolor) in self.region_colors:
            ...
            ax.annotate('{} Days '.format(day), xy=(day-1,y_max), rotation='vertical', fontsize=7, color='#666666', ha='right', va='top')
    ...
    def plot_four(self, axis, column, color1, color2, last=False, tick_right=False):
        ...
        for prefix,style in [('','solid'), ('world_', 'dotted')]:
            for col,color in [('host_count',color1), ('vulnerable_host_count', color2)]:
                ...
                df = df.fillna(0)
                df.plot(ax=axis, label=prefix+column, grid=False, color=color, linewidth=2, linestyle=style, marker='.', markersize=10)
