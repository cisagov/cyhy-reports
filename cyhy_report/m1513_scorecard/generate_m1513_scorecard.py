...
class ScorecardGenerator(object):
        ...
        for domain_doc in all_domains_cursor:
            ...
            if domain_doc["base_domain"] == domain_doc["domain"]:
                domain_doc["subdomains"] = list(
                    self.__db.m1513.find(
                        {
                            "base_domain": domain_doc["base_domain"],
                            "_id": {"$ne": domain_doc["_id"]},
                        }
                    ).sort([("domain", 1)])
                )
    ...
    def __score_domain(self, domain):
        ...
        if domain["live"] == "True":
            ...
            if domain["domain_supports_https"] == "True":
                ...
                score[
                    "domain_supports_https_bool"
                ] = False  # print("valid no "+ score['domain'])  #debug
            ...
            # https_bad_chain
            if (
                domain["https_bad_chain"] == "True"
                and domain["https_bad_hostname"] == "True"
            ):
                score["https_bad_chain_bool"] = True
                self.__https_bad_chain_count += 1
            elif (
                domain["https_bad_chain"] == "True"
                and domain["https_bad_hostname"] == "False"
            ) or (
                domain["https_bad_chain"] == "True"
                and domain["https_expired_cert"] == "True"
            ):
                self.__https_bad_chain_count += 1
                # self.__domain_supports_https_count += 1
            ...
            # M-15-13 compliant?
            if (
                domain["domain_supports_https"] == "True"
                and domain["domain_enforces_https"] == "True"
                and domain["domain_uses_strong_hsts"] == "True"
            ):
            ...
            if domain.get("subdomains"):  # if this domain has any subdomains
                for subdomain in domain["subdomains"]:
                    ...
                    if subdomain_score:
                        score["subdomain_scores"].append(
                            subdomain_score
                        )  # add this subdomain's score to this domain's list of subdomain_scores
            return score
    ...
    def __populate_scorecard_doc(self):
        ...
        for domain in self.__base_domains:
            ...
            if score:
                self.__scorecard_doc["scores"].append(
                    score
        ...
        self.__enforces_https_percentage = round(
            (
                (self.__domain_enforces_https_count / self.__all_eligible_domains_count)
                * 100
            ),
            1,
        )
        ...
        self.__m1513_percentage = round(
            ((self.__m1513_count / self.__all_eligible_domains_count) * 100), 1
        )
    ...
    def __generate_attachments(self):
        ...
        bashCommand = (
            "bash /home/report/exporting_agency.sh " + '"' + self.__agency + '"'
        )
    ...
    def __generate_https_attachment(self):
        ...
        header_fields = (
            ...
            "Registered Domains",
            "Found Subdomains",
            "Web-responsive Domains",
            "Uses HTTPS",
            "Enforces HTTPS",
            "HSTS",
            "Preloaded",
            "M-15-13 Compliant Domains",
        )
        data_fields = (
            "Agency",
            "Registered Domains",
            ...
            data_writer.writerow(
                {
                    ...
                    "Registered Domains": self.__base_domain_count,
                    "Found Subdomains": self.__subdomain_count,
                    "Web-responsive Domains": self.__all_eligible_domains_count,
                    "Uses HTTPS": self.__domain_supports_https_count,
                    "Enforces HTTPS": self.__strictly_forces_count,
                    "HSTS": self.__hsts_count,
                    "Preloaded": self.__hsts_preloaded_count,
                    "M-15-13 Compliant Domains": self.__m1513_count,
                }
            )
    ...
    def __figure_overview(self):
        ...
        Diff = (
            100 - self.__uses_https_percentage,
            100 - self.__enforces_https_percentage,
            100 - self.__hsts_percentage,
        )
        ...
        # plt.show()
        plt.savefig(
            self.__path + "/component-compliance"
    ...
    def __donut_figure(self):
        ...
        plt.pie(
            sizes, labels=labels, colors=colors, shadow=False, startangle=90
        )  # autopct='%1.1f%%'
        ...
        # plt.show()
        plt.savefig(
            self.__path + "/overall-compliance"
    ...
    def __generate_mustache_json(self, filename):
        ...
        result["strictly_forces_percentage"] = round(
            ((self.__strictly_forces_count / self.__domain_count) * 100), 1
        )
        result["downgrades_percentage"] = round(
            ((self.__downgrades_count / self.__domain_count) * 100), 1
        )
        result["hsts_percentage"] = self.__hsts_percentage
        result["hsts_preloaded_percentage"] = round(
            ((self.__hsts_preloaded_count / self.__domain_count) * 100), 1
        )
        result["hsts_entire_domain_percentage"] = round(
            ((self.__hsts_entire_domain_count / self.__domain_count) * 100), 1
        )
        result["m1513_percentage"] = self.__m1513_percentage
        result["m1513_count"] = int(self.__m1513_count)
        result["domain_supports_https_count"] = int(
            self.__domain_supports_https_count
        )  # added
        ...
        result["domain_uses_strong_hsts_count"] = int(
            self.__domain_uses_strong_hsts_count
        )
