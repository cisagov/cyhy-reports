...
OCSP_URL = (
    "https://raw.githubusercontent.com/GSA/data/master/dotgov-websites/ocsp-crl.csv"
)
OCSP_FILE = "/tmp/ocsp-crl.csv"

TRIPLE_DES_EXCEPTIONS_URL = "https://raw.githubusercontent.com/cisagov/scan-target-data/develop/3des-exception-agencies.csv"
...
class ScorecardGenerator(object):
    ...
    def __open_tix_opened_in_date_range_pl(
        self, severity, current_date, days_until_tix_overdue
    ):
        return [
            {
                "$match": {
    ...
    def __open_tix_opened_in_date_range_for_orgs_pl(
        self,
        severity,
        current_date,
        parent_org,
    ...
    def __active_hosts_pl(self):
        return [
            {"$match": {"state.up": True}},
            {"$group": {"_id": {"owner": "$owner"}, "active_hosts_count": {"$sum": 1}}},
    ...
    def __load_ticket_age_data(
        self, start_date, severity, graph_bucket_cutoff_days, org_list
    ):
        ...
        if len(tix):
            ...
            for start_of_day, values in results_df.iterrows():
                ...
                age_on_date_masked = age_on_date.mask(open_on_day_mask is False)
    ...
    def __load_open_tickets(self, severity, org_list):
        return list(
            self.__cyhy_db.tickets.find(
                {
                    "open": True,
                    "owner": {"$in": self.__all_cybex_orgs_with_descendants},
                    "details.severity": severity,
                    "false_positive": False,
                },
                {"time_opened": 1},
    ...
    def __run_vuln_scan_queries(self, cybex_orgs):
        ...
        for r in self.__requests:
            if r.get("children"):
                ...
                self.__results["vuln-scan"]["addresses"].append(
                    {"_id": {"owner": r["_id"]}, "addresses_count": len(r.networks)}
                )

        # Get relevant critical-severity ticket data
        pipeline_collection = self.__open_tix_opened_in_date_range_pl(
            CRITICAL_SEVERITY, self.__generated_time, DAYS_UNTIL_OVERDUE_CRITICAL
        )
        self.__results["vuln-scan"][
            "open_critical_ticket_counts"
        ] = database.run_pipeline_cursor(pipeline_collection, self.__cyhy_db)

        # Get relevant high-severity ticket data
        pipeline_collection = self.__open_tix_opened_in_date_range_pl(
            HIGH_SEVERITY, self.__generated_time, DAYS_UNTIL_OVERDUE_HIGH
        )
        self.__results["vuln-scan"][
        ...
        self.__results["vuln-scan"]["active_hosts"] = database.run_pipeline_cursor(
            pipeline_collection, self.__cyhy_db
        )
        ...
        for r in requests_with_descendants:
            ...
            pipeline_collection = self.__open_tix_opened_in_date_range_for_orgs_pl(
                CRITICAL_SEVERITY,
            ...
            for descendant_id in descendants:
                address_count += len(
                    self.__cyhy_db.RequestDoc.get_by_owner(descendant_id).networks
                )
            self.__results["vuln-scan"]["addresses"].append(
                {"_id": {"owner": r["_id"]}, "addresses_count": address_count}
            )
            ...
            for i in self.__scan_db.trustymail.find(
                {
                    "latest": True,
                    "agency.id": {"$in": cybex_orgs},
                    "$or": [{"is_base_domain": True}, {"domain_supports_smtp": True}],
                },
                {"_id": 0, "domain": 1},
            )
        ]
                ...
                [
                    ...
                    },
                    {
                        "$project": {
                            ...
                            "dmarc_subdomain_policy": {
                                "$ifNull": ["$dmarc_subdomain_policy", "reject"]
                            },
                            "dmarc_policy_percentage": "$dmarc_policy_percentage",
                            "valid_dmarc": "$valid_dmarc",
                    ...
                    },
                    {
                        "$group": {
                            ...
                            "no_dmarc_record": {
                                "$sum": {
                                    "$cond": [{"$eq": ["$dmarc_record", False]}, 1, 0]
                                }
                            },
                        }
                    },
                ...
                [
                    {
                        "$match": {
                            ...
                            "domain": {
                                "$in": self.__results[
                                    "latest_cybex_https_scan_live_hostnames"
                                ]
                            },
                        }
                    },
    ...
    def __run_sslyze_scan_queries(self, cybex_orgs):
        ...
        self.__results["sslyze-scan"] = list(
            self.__scan_db.sslyze_scan.aggregate(
                [
                    {
                        "$match": {
                            "latest": True,
                            "domain": {"$in": latest_cybex_hostnames},
                            "scanned_port": {"$in": [25, 587, 465, 443]},
                        }
                    ...
                    {
                        "$group": {
                            "_id": {"domain": "$domain", "agency_id": "$agency_id"},
    ...
    def __create_domain_to_org_map(self, org_list):
        ...
        domains = self.__scan_db.domains.find(
            {"agency.id": {"$in": org_list}}, {"_id": True, "agency.id": True}
        )

        return {d["_id"].lower(): d["agency"]["id"] for d in domains}

    def __accumulate_federal_cert_totals(self, certificate, field_to_accumulate):
        ...
        if certificate["non_cfo_act_org"]:
            self.__results["non_cfo_totals"]["cert-scan"][field_to_accumulate] += 1
    ...
    def __create_cert_summary_by_org(self, certificates, current_fy_start):
        ...
        for cert in certificates:
            ...
            if cert["not_after"] > self.__generated_time:
                ...
                self.__accumulate_federal_cert_totals(cert, "unexpired_certs_count")
    ...
    def __run_cert_scan_queries(self, cybex_orgs):
        ...
        self.__results["domain_to_org_map"] = self.__create_domain_to_org_map(
            cybex_orgs
        )
        ...
        relevant_certs = self.__scan_db.certs.find(
            {
                "trimmed_subjects": {"$in": cybex_domains},
                "$or": [
        ...
        self.__results["cert-scan"] = self.__create_cert_summary_by_org(
            relevant_certs, current_fy_start
        )

    def __run_queries(self):
        # Get cyhy request docs for all orgs that have CYBEX in their report_types
        self.__requests = list(
            self.__cyhy_db.RequestDoc.find({"report_types": REPORT_TYPE.CYBEX})
        )
        cybex_orgs = []
        for r in self.__requests:
            ...
            self.__all_cybex_orgs_with_descendants += (
                self.__cyhy_db.RequestDoc.get_all_descendants(r["_id"])
            )
        ...
        for tally in list(self.__cyhy_db.TallyDoc.find({"_id": {"$in": cybex_orgs}})):
            if tally["last_change"] >= self.__generated_time - timedelta(
                ...
                tally_descendant_orgs = self.__cyhy_db.RequestDoc.get_all_descendants(
                    tally["_id"]
                )
                if tally_descendant_orgs:
                    for tally_descendant in list(
                        self.__cyhy_db.TallyDoc.find(
                            {"_id": {"$in": tally_descendant_orgs}}
                        )
                    ):
                        if tally_descendant[
                            "last_change"
        ...
        if self.__cyhy_db.RequestDoc.find_one({"_id": "FED_CFO_ACT"}):
            self.__cfo_act_orgs = self.__cyhy_db.RequestDoc.find_one(
                {"_id": "FED_CFO_ACT"}
            )["children"]
        else:
            self.__cfo_act_orgs = []
    ...
    def __populate_scorecard_doc(self):
        ...
        for r in self.__requests:
            score = {
                ...
                "trustymail": {
                    ...
                    "base_domains": {
                        ...
                        "live_supports_starttls_pct": 0.0,
                        "live_supports_starttls_pct_str": "0.0%",
                        "all_live_supports_starttls": False,
                        "live_missing_starttls_count": 0,
                        "live_valid_spf_count": 0,
                        "live_valid_spf_pct": 0.0,
                        "live_valid_spf_pct_str": "0.0%",
                        "live_spf_covered_count": 0,
                        "live_spf_covered_pct": 0.0,
                        "live_spf_covered_pct_str": "0.0%",
                        "all_live_spf_covered": False,
                        "live_valid_dmarc_count": 0,
                        "live_valid_dmarc_pct": 0.0,
                        "live_valid_dmarc_pct_str": "0.0%",
                        "all_live_dmarc_valid": False,
                        "live_dmarc_reject_count": 0,
                        "live_dmarc_reject_pct": 0.0,
                        "live_dmarc_reject_pct_str": "0.0%",
                        "all_live_dmarc_reject": False,
                        "live_has_bod1801_dmarc_uri_count": 0,
                        "live_has_bod1801_dmarc_uri_pct": 0.0,
                        "live_has_bod1801_dmarc_uri_pct_str": "0.0%",
                        ...
                        "live_no_weak_crypto_pct": 0,
                        "live_no_weak_crypto_pct_str": "0.0%",
                        "all_live_no_weak_crypto": False,
                        "live_bod1801_email_compliant_count": 0,
                        "live_bod1801_email_compliant_pct": 0.0,
                        "live_bod1801_email_compliant_pct_str": "0.0%",
            ...
            for trustymail_result_set in [
                "base_domains",
                "base_domains_and_smtp_subdomains",
            ]:
                for trustymail_result in self.__results[
                    "trustymail_" + trustymail_result_set
                ]:
                    if (
                        trustymail_result["_id"] == score["owner"]
                        ...
                        for metric in (
                            ...
                            score["trustymail"][trustymail_result_set][
                                metric
                            ] = trustymail_result[metric]

                        # Calculate trustymail summary percentages
                        if trustymail_result["live_domain_count"]:
                            current_live_domain_count = trustymail_result[
                                "live_domain_count"
                            ]
                            for metric in [
                                "live_valid_dmarc",
                                "live_dmarc_reject",
                                "live_has_bod1801_dmarc_uri",
                                "live_supports_starttls",
                                "live_spf_covered",
                                "live_no_weak_crypto",
                                "live_bod1801_email_compliant",
                            ]:
                                current_metric_count = trustymail_result[
                                    metric + "_count"
                                ]
                                score["trustymail"][trustymail_result_set][
                                    metric + "_pct"
                                ] = current_metric_count / float(
                                    current_live_domain_count
                                )
                                score["trustymail"][trustymail_result_set][
                                    metric + "_pct_str"
                                ] = "{0:.1%}".format(
                        ...
                        for (score_percent, perfect_flag) in [
                            ("live_valid_dmarc_pct", "all_live_dmarc_valid"),
                            ("live_dmarc_reject_pct", "all_live_dmarc_reject"),
                            (
                                "live_has_bod1801_dmarc_uri_pct",
                                "all_live_has_bod1801_dmarc_uri",
                            ),
                            (
                                "live_supports_starttls_pct",
                                "all_live_supports_starttls",
                            ),
                            ("live_spf_covered_pct", "all_live_spf_covered"),
                            ("live_no_weak_crypto_pct", "all_live_no_weak_crypto"),
                            (
                                "live_bod1801_email_compliant_pct",
                                "all_live_bod1801_email_compliant",
                            ),
                        ]:
                            if (
                                score["trustymail"][trustymail_result_set][
                        ...
                        if trustymail_result["live_dmarc_reject_count"] == 0:
                            ...
                            if (
                                trustymail_result_set
                                == "base_domains_and_smtp_subdomains"
                            ):
                                self.__dmarc_reject_none.append(
                                    {
                                        "acronym": score["owner"],
                            ...
                            trustymail_result["live_dmarc_reject_count"]
                            == trustymail_result["live_domain_count"]
                        ):
                            score["trustymail"][trustymail_result_set][
                                "dmarc_reject_all"
                            ] = True
                            ...
                            if (
                                ...
                                self.__dmarc_reject_all.append(
                                    {
                                        "acronym": score["owner"],
                                        "cfo_act_org": score["cfo_act_org"],
                                    }
                            ...
                            if (
                                ...
                                self.__dmarc_reject_some.append(
                                    {
                                        "acronym": score["owner"],
                                        "cfo_act_org": score["cfo_act_org"],
                                    }
            ...
            for https_scan_result in self.__results["https-scan"]:
                if (
                    https_scan_result["_id"] == score["owner"]
                ):  # Found info for the current org
                    ...
                    for metric in (
                        ...
                        score["https-scan"]["live_domains"][metric] = https_scan_result[
                            metric
                        ]

                    # Calculate https-scan percentages
                    if https_scan_result["live_domain_count"]:
                        current_live_domain_count = https_scan_result[
                            "live_domain_count"
                        ]
                        for metric in [
                            "live_supports_https",
                            "live_enforces_https",
                            "live_uses_strong_hsts",
                            "live_no_weak_crypto",
                            "live_bod1801_web_compliant",
                        ]:
                            current_metric_count = https_scan_result[metric + "_count"]
                            score["https-scan"]["live_domains"][
                                metric + "_pct"
                            ] = current_metric_count / float(current_live_domain_count)
                            score["https-scan"]["live_domains"][
                                metric + "_pct_str"
                            ] = "{0:.1%}".format(
                                score["https-scan"]["live_domains"][metric + "_pct"]
                            )

                    # Check for perfect scores in each category to be displayed in the 'Results by Agency' section
                    for (score_percent, perfect_flag) in [
                        ("live_supports_https_pct", "all_live_supports_https"),
                        ...
                        (
                            ...
                            "all_live_bod1801_web_compliant",
                        ),
                    ]:
                    ...
                    if https_scan_result["live_uses_strong_hsts_count"] == 0:
                        ...
                        https_scan_result["live_uses_strong_hsts_count"]
                        == https_scan_result["live_domain_count"]
                    ):
            ...
            for sslyze_scan_result in self.__results["sslyze-scan"]:
                if (
                    sslyze_scan_result["_id"] == score["owner"]
                ):  # Found info for the current org
                    score["sslyze-scan"]["scanned"] = True
                    score["sslyze-scan"]["live_domains"][
                        "domain_count"
                    ] = sslyze_scan_result["domain_count"]
                    score["sslyze-scan"]["live_domains"][
                        "live_has_weak_crypto_count"
                    ] = sslyze_scan_result["domains_with_weak_crypto_count"]
                    score["sslyze-scan"]["live_domains"][
                        "live_no_weak_crypto_count"
                    ] = (
                        sslyze_scan_result["domain_count"]
            ...
            if self.__results["cert-scan"].get(
                score["owner"]
            ):  # Found info for the current org
                ...
                for metric in [
                    ...
                    score["cert-scan"]["metrics"][metric] = self.__results["cert-scan"][
                        score["owner"]
                    ][metric]
            ...
            for t in self.__tallies:
                if (
                    t["_id"] == r["_id"]
                ):  # Found a current CyHy tally that matches this request (org)
                    ...
                    for i in self.__previous_scorecard_data["all_orgs_alpha"]:
                        if (
                            ...
                            if i["vuln-scan"]["metrics"].get("open_criticals"):
                                score["vuln-scan"]["metrics"][
                                    "open_criticals_on_previous_scorecard"
                                ] = i["vuln-scan"]["metrics"]["open_criticals"]

                            if i["vuln-scan"]["metrics"].get("open_highs"):
                                score["vuln-scan"]["metrics"][
                                    "open_highs_on_previous_scorecard"
                                ] = i["vuln-scan"]["metrics"]["open_highs"]
                            break

                    # Search through CyHy query results for data from the current org and add it to the current score
                    for vuln_scan_result in self.__results["vuln-scan"][
                        "open_critical_ticket_counts"
                    ]:
                        if (
                            vuln_scan_result["_id"]["owner"] == score["owner"]
                        ):  # Found info for the current org
                    ...
                    for (result_field, score_field, data_key) in [
                        ("addresses", "addresses", "addresses_count"),
                        ("active_hosts", "active_hosts", "active_hosts_count"),
                    ]:
                        for i in self.__results["vuln-scan"][result_field]:
                            if (
                                i["_id"]["owner"] == score["owner"]
                            ):  # Found info for the current org
                                score["vuln-scan"]["metrics"][score_field] = i[data_key]
                                break

                    # Fields calculated from info retrieved above
                    score["vuln-scan"]["metrics"]["open_criticals"] = (
                        score["vuln-scan"]["metrics"]["open_criticals"]
                        - score["vuln-scan"]["metrics"]["open_criticals_0-7_days"]
                    )  # Hack for CYHY-441; exclude criticals less than 7 days old from the open_criticals total
                    score["vuln-scan"]["metrics"][
                    ...
                    score["vuln-scan"]["metrics"]["open_highs"] = (
                        score["vuln-scan"]["metrics"]["open_highs"]
                        - score["vuln-scan"]["metrics"]["open_highs_0-7_days"]
                    )  # Hack for CYHY-441; exclude criticals less than 7 days old from the open_criticals total
                    score["vuln-scan"]["metrics"][
    ...
    def __calculate_federal_totals(self):
        ...
        for total_id in ["federal_totals", "cfo_totals", "non_cfo_totals"]:
            ...
            self.__results[total_id]["vuln-scan"] = {
                "metrics": {
                    "open_criticals": 0,
                    "open_criticals_on_previous_scorecard": 0,
                    "open_criticals_0-7_days": 0,
            ...
            for trustymail_result_set in [
                ...
                self.__results[total_id]["trustymail"][trustymail_result_set] = {
                    "domain_count": 0,
                    "live_domain_count": 0,
                    "live_valid_dmarc_count": 0,
                    "live_dmarc_reject_count": 0,
            ...
            self.__results[total_id]["https-scan"] = {
                "live_domains": {
                    "live_domain_count": 0,
                    "live_supports_https_count": 0,
                    "live_enforces_https_count": 0,
            ...
            self.__results[total_id]["sslyze-scan"] = {
                "live_domains": {
                    "domain_count": 0,
                    "live_no_weak_crypto_count": 0,
                    "live_has_weak_crypto_count": 0,
        ...
        for org in self.__scorecard_doc["scores"]:
            for (scanner, scan_subtype, field) in [
                ("vuln-scan", "metrics", "open_criticals"),
                ...
                self.__results["federal_totals"][scanner][scan_subtype][field] += org[
                    scanner
                ][scan_subtype][field]
                if org["cfo_act_org"]:
                    self.__results["cfo_totals"][scanner][scan_subtype][field] += org[
                        scanner
                    ][scan_subtype][field]
                else:
                    self.__results["non_cfo_totals"][scanner][scan_subtype][
                        field
                    ] += org[scanner][scan_subtype][field]

        for total_id in ["federal_totals", "cfo_totals", "non_cfo_totals"]:
            # Calculate open_criticals_delta_since_last_scorecard
            self.__results[total_id]["vuln-scan"]["metrics"][
                "open_criticals_delta_since_last_scorecard"
            ] = (
                self.__results[total_id]["vuln-scan"]["metrics"]["open_criticals"]
                - self.__results[total_id]["vuln-scan"]["metrics"][
                    "open_criticals_on_previous_scorecard"
                ]
            )
            ...
            if self.__results[total_id]["vuln-scan"]["metrics"].get(
                ...
                self.__results[total_id]["vuln-scan"]["metrics"][
                    ...
                    self.__results[total_id]["vuln-scan"]["metrics"]["open_highs"]
                    - self.__results[total_id]["vuln-scan"]["metrics"][
                        "open_highs_on_previous_scorecard"
                    ]
                )
            ...
            for (scanner, scan_subtype, metric) in [
                ("trustymail", "base_domains", "live_supports_starttls"),
                ...
                current_metric_count = self.__results[total_id][scanner][scan_subtype][
                    metric + "_count"
                ]
                current_live_domain_count = self.__results[total_id][scanner][
                    scan_subtype
                ]["live_domain_count"]
                if current_live_domain_count:
                    self.__results[total_id][scanner][scan_subtype][
                        metric + "_pct"
                    ] = current_metric_count / float(current_live_domain_count)
                    self.__results[total_id][scanner][scan_subtype][
                        metric + "_pct_str"
                    ] = "{0:.1%}".format(
                        self.__results[total_id][scanner][scan_subtype][metric + "_pct"]
                    )
                    self.__results[total_id][scanner][scan_subtype][
                        metric + "_pct_int"
                    ] = int(
                        round(
                            ...
                            / float(current_live_domain_count)
                            * 100
                        )
                    )  # In python 3, int/float stuff won't be needed
                else:
                    self.__results[total_id][scanner][scan_subtype][
                        metric + "_pct"
                    ] = 0.0
    ...
    def __make_fake_agency(
        ...
        while not acceptable_name:
            ...
            if (fake_name not in real_agencies + fake_agencies) and (
                fake_acronym not in real_acronyms + fake_acronyms + bad_acronyms
            ):
        ...
        for score_list in [
            ...
            for s in score_list:
                ...
                if real_to_fake_lookup.get(real_acronym):
                    ...
                    fake_agency_name, fake_agency_acronym = self.__make_fake_agency(
                        real_agency_names,
                        real_agency_acronyms,
                        fake_agency_names,
                        fake_agency_acronyms,
                    ...
                    real_to_fake_lookup[s["acronym"]] = {
                        "fake_acronym": fake_agency_acronym,
                        "fake_name": fake_agency_name,
                    }
    ...
    def generate_cybex_scorecard(self):
        ...
        if self.__anonymize:
            ...
            self.__log_scorecard_to_db = (
                False  # Don't log creation of anonymous scorecards to the DB
            )
        ...
        if not self.__debug:
            ...
            dest_filename = self.__results[
                "scorecard_name"
            ] + "_Cyber_Exposure_Scorecard-%s.pdf" % (timestamp)
    ...
    def __generate_ed1901_results_by_agency_attachment(self):
        header_fields = (
            ...
            "new_certificates_current_fiscal_year",
            "new_certificates_past_30_days",
            "new_certificates_past_7_days",
        )
        data_fields = (
            "acronym",
            "name",
            ...
            for org in copy.deepcopy(self.__scorecard_doc["scores"]):
                for cert_scan_key in (
                    ...
                    if org["cert-scan"]["scanned"]:
                        org[cert_scan_key] = org["cert-scan"]["metrics"].get(
                            cert_scan_key
                        )
                    else:
    ...
    def __generate_email_security_summary_attachment(self):
        trustymail_dmarc_summary = sorted(
            self.__results["trustymail_dmarc_summary"], key=lambda x: x["_id"]
        )
        with open(EMAIL_SECURITY_SUMMARY_CSV_FILE, "wb") as out_file:
            ...
            for (row_title, summary_field) in [
                ("p=none", "dmarc_policy_none"),
                ("p=quarantine", "dmarc_policy_quarantine"),
                ("policy_of_reject", "dmarc_policy_reject"),
                ("reports_dmarc_to_cisa", "dmarc_correct_rua"),
                ("invalid_dmarc_record", "invalid_dmarc_record"),
                ("no_dmarc_record", "no_dmarc_record"),
                ("domains_tested", "base_domain_count"),
            ]:
                data_row = [row_title]
                for summary_item in trustymail_dmarc_summary:
    ...
    def __generate_bod_results_by_agency_attachment(self):
        header_fields = (
            ...
            "overdue_critical_vulns_{}+_days".format(DAYS_UNTIL_OVERDUE_CRITICAL),
            "active_high_vulns",
            "overdue_high_vulns_{}+_days".format(DAYS_UNTIL_OVERDUE_HIGH),
        ...
        with open(BOD_RESULTS_BY_AGENCY_CSV_FILE, "wb") as out_file:
            ...
            for org in copy.deepcopy(self.__scorecard_doc["scores"]):
                ...
                if org["https-scan"]["scanned"]:
                    org["live_bod1801_web_compliant_pct"] = org["https-scan"][
                        "live_domains"
                ...
                if org["trustymail"]["scanned"]:
                    org["live_bod1801_email_compliant_pct"] = org["trustymail"][
                        "base_domains_and_smtp_subdomains"
    ...
    def __generate_bod1501_results_by_agency_attachment(self):
        ...
        header_fields = (
            ...
            "cfo_act",
            "active_critical_vulns",
            "delta_active_critical_vulns_since_" + prev_scorecard_date_txt,
            ...
            "active_high_vulns",
            "delta_active_high_vulns_since_" + prev_scorecard_date_txt,
    ...
    def __generate_web_security_results_by_agency_attachment(self):
        header_fields = (
            ...
            "uses_strong_hsts",
            "uses_strong_hsts_%",
            "free_of_sslv2/v3,3des,rc4",
            "free_of_sslv2/v3,3des,rc4_%",
            "bod_18-01_web_compliant",
            "bod_18-01_web_compliant_%",
        )
        data_fields = (
            "acronym",
            ...
            "live_domain_count",
            "live_supports_https_count",
            "live_supports_https_pct",
            "live_enforces_https_count",
            "live_enforces_https_pct",
            "live_uses_strong_hsts_count",
            "live_uses_strong_hsts_pct",
            "live_no_weak_crypto_count",
            "live_no_weak_crypto_pct",
            "live_bod1801_web_compliant_count",
            "live_bod1801_web_compliant_pct",
        )
        with open(WEB_SECURITY_RESULTS_BY_AGENCY_CSV_FILE, "wb") as out_file:
            ...
            for org in copy.deepcopy(self.__scorecard_doc["scores"]):
                for https_scan_key in (
                    ...
                    if org["https-scan"]["scanned"]:
                        org[https_scan_key] = org["https-scan"]["live_domains"].get(
                            https_scan_key
                        )
                    else:
    ...
    def __generate_email_security_results_by_agency_attachment(self):
        header_fields = (
            ...
            "name",
            "cfo_act",
            "live_domains_and_smtp_subdomains",
            "valid_dmarc_record",
            "valid_dmarc_record_%",
            "dmarc_policy_of_reject",
            "dmarc_policy_of_reject_%",
            "reports_dmarc_to_cisa",
            "reports_dmarc_to_cisa_%",
            "supports_starttls",
            "supports_starttls_%",
            "has_spf_covered",
            "has_spf_covered_%",
        ...
        data_fields = (
            ...
            "live_domain_count",
            "live_valid_dmarc_count",
            "live_valid_dmarc_pct",
            "live_dmarc_reject_count",
            "live_dmarc_reject_pct",
            "live_has_bod1801_dmarc_uri_count",
            "live_has_bod1801_dmarc_uri_pct",
            "live_supports_starttls_count",
            "live_supports_starttls_pct",
            "live_spf_covered_count",
            "live_spf_covered_pct",
            "live_no_weak_crypto_count",
            "live_no_weak_crypto_pct",
            "live_bod1801_email_compliant_count",
            "live_bod1801_email_compliant_pct",
        ...
        with open(EMAIL_SECURITY_RESULTS_BY_AGENCY_CSV_FILE, "wb") as out_file:
            ...
            for org in copy.deepcopy(self.__scorecard_doc["scores"]):
                for trustymail_key in (
                    ...
                    if org["trustymail"]["scanned"]:
                        org[trustymail_key] = org["trustymail"][
                            "base_domains_and_smtp_subdomains"
                        ].get(trustymail_key)
                    else:
    ...
    ###############################################################################
    def __figure_vuln_age_history_graph(
        self, ticket_age_dataframe, figure_filename, severity_label
    ):
        if len(ticket_age_dataframe):
            line = graphs.MyStackedLine(
                ticket_age_dataframe,
                ylabel="{} Vulnerabilities".format(severity_label),
                data_labels=["Active Less Than 30 Days", "Active 30+ Days"],
                data_fill_colors=["#0099cc", "#cc0000"],
            )
            line.plot(figure_filename, size=1.6)
        else:
            message = graphs.MyMessage(
                "No {} Vulnerabilities To Display\nFigure Omitted".format(
                    severity_label
                )
            )
            message.plot(figure_filename, size=1.0)

    def __figure_vuln_age_distribution(
        self,
        tickets,
        figure_filename,
        severity_label,
        ...
        if len(age_buckets):
            ...
            region_colors = [
                (ticket_age_buckets[0][1], "#ffffb2"),
                (ticket_age_buckets[1][1], "#fecc5c"),
                (ticket_age_buckets[2][1], "#fd8d3c"),
                (ticket_age_buckets[3][1], "#f03b20"),
                (ticket_age_buckets[4][1], "#bd0026"),
            ]  # Colorize regions
    ...
    def __figure_bod1801_email_components(self):
        ...
        bod_1801_email_bar = graphs.MyTrustyBar(
            percentage_list=[
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_valid_dmarc_pct_int"],
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_dmarc_reject_pct_int"],
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_has_bod1801_dmarc_uri_pct_int"],
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_supports_starttls_pct_int"],
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_spf_covered_pct_int"],
                self.__results["federal_totals"]["trustymail"][
                    "base_domains_and_smtp_subdomains"
                ]["live_no_weak_crypto_pct_int"],
            ],
            label_list=[
                "Valid\nDMARC",
                "DMARC\nPolicy of Reject",
    ...
    def __figure_bod1801_email_compliant(self):
        donut = graphs.MyDonutPie(
            percentage_full=self.__results["federal_totals"]["trustymail"][
                "base_domains_and_smtp_subdomains"
            ]["live_bod1801_email_compliant_pct_int"],
            label="BOD 18-01\nCompliant\n(Email)",
            fill_color=graphs.DARK_BLUE,
        )
    ...
    def __figure_bod1801_web_components(self):
        ...
        bod_1801_web_bar = graphs.MyTrustyBar(
            percentage_list=[
                self.__results["federal_totals"]["https-scan"]["live_domains"][
                    "live_supports_https_pct_int"
                ],
                self.__results["federal_totals"]["https-scan"]["live_domains"][
                    "live_enforces_https_pct_int"
                ],
                self.__results["federal_totals"]["https-scan"]["live_domains"][
                    "live_uses_strong_hsts_pct_int"
                ],
                self.__results["federal_totals"]["https-scan"]["live_domains"][
                    "live_no_weak_crypto_pct_int"
                ],
    ...
    def __figure_bod1801_web_compliant(self):
        donut = graphs.MyDonutPie(
            percentage_full=self.__results["federal_totals"]["https-scan"][
                "live_domains"
            ]["live_bod1801_web_compliant_pct_int"],
            label="BOD 18-01\nCompliant\n(Web)",
            fill_color=graphs.DARK_BLUE,
        )
    ...
    def __generate_mustache_json(self, filename):
        ...
        result[
            "orgs_with_recently_issued_certs"
        ] = self.__orgs_with_recently_issued_certs
        result[
            "orgs_with_no_recently_issued_certs"
        ] = self.__orgs_with_no_recently_issued_certs
        result["orgs_with_no_known_domains"] = self.__orgs_with_no_known_domains
        result["all_orgs_ed1901_cert"] = sorted(
            self.__scorecard_doc["scores"],
            key=lambda x: (
                x["cert-scan"]["metrics"].get("certs_issued_past_7_days_count"),
                x["cert-scan"]["metrics"].get("certs_issued_past_30_days_count"),
                x["cert-scan"]["metrics"].get("certs_issued_current_fy_count"),
                x["cert-scan"]["metrics"].get("unexpired_certs_count"),
                x["cert-scan"].get("scanned"),
            ),
            reverse=True,
        )
        ...
        result[
            "orgs_without_criticals_or_highs"
        ] = self.__orgs_without_criticals_or_highs
        result["orgs_not_vuln_scanned"] = self.__orgs_not_vuln_scanned
        result["overall_bod_orgs"] = sorted(
            self.__scorecard_doc["scores"],
            key=lambda x: (
                ...
                x["https-scan"]["live_domains"].get("live_bod1801_web_compliant_pct"),
        ...
        result["all_orgs_bod1801_email_compliant"] = sorted(
            self.__scorecard_doc["scores"],
            key=lambda x: (
                x["trustymail"]["base_domains_and_smtp_subdomains"].get(
                    "live_bod1801_email_compliant_pct"
        ...
        result["dmarc_summary"] = sorted(
            self.__results["trustymail_dmarc_summary"], key=lambda x: x["_id"]
        )
        for dmarc_summary_item in result["dmarc_summary"]:
            dmarc_summary_item["scan_date_tex"] = dmarc_summary_item["_id"].strftime(
                "{%d}{%m}{%Y}"
            )

        result["all_orgs_bod1801_web_compliant"] = sorted(
            self.__scorecard_doc["scores"],
            key=lambda x: (
                x["https-scan"]["live_domains"].get("live_bod1801_web_compliant_pct"),
                x["https-scan"]["live_domains"].get("live_uses_strong_hsts_pct"),
                x["https-scan"]["live_domains"].get("live_enforces_https_pct"),
                x["https-scan"]["live_domains"].get("live_supports_https_pct"),
                x["https-scan"]["live_domains"].get("live_no_weak_crypto_pct"),
                x["https-scan"]["live_domains"].get("live_domain_count"),
            ),
            reverse=True,
        )
        ...
        CYHY_HOST_MAX_RESCAN_FREQUENCY_DAYS = scheduler.DefaultScheduler.PRIORITY_TIMES[
            -1
        ]
        ...
        for x in (
            self.__results["trustymail_base_domains"] + self.__results["https-scan"]
        ):
        ...
        if self.__log_scorecard_to_db:
            ...
            result[
                "scorecard_oid"
            ] = None  # If scorecard_oid is None, it will not be included in the PDF metadata
...
def generate_empty_scorecard_json():
    ...
    empty_totals = {
        ...
        "trustymail": {"base_domains": {}, "base_domains_and_smtp_subdomains": {}},
...
def main():
    ...
    if cyhy_db.RequestDoc.find_one({"report_types": REPORT_TYPE.CYBEX}):
        ...
        print 'No organizations found in database with "{}" report type - exiting!'.format(
            REPORT_TYPE.CYBEX
        )
    sys.exit(0)
