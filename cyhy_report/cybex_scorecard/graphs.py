...
class MyTrustyBar(object):
    ...
    def plot(self, filename):
        x_left_indices = np.arange(
            len(self.percentage_list)
        ...
        p2 = plt.bar(
            x_left_indices,
            [100 - x for x in self.percentage_list],
            width,
            color="w",
            bottom=self.percentage_list,
            edgecolor="none",
        )

        plt.ylabel("Percent (%)", fontsize=14, style="italic")
        if self.title:
            # put title text a bit higher so it avoids label text of 100% bars
            plt.title(self.title, fontsize=20, fontweight="bold", y=1.07)

        # Hack below for centering ticks in matplotlib 1.5.3 - not needed in matplotlib 2.x
        plt.xticks(
            x_left_indices + width / 2.0, self.label_list, fontsize=14, style="italic"
        )
        ...
        # plt.show()
        plt.tight_layout()  # trims margins down nicely, at least in matplotlib 1.5.3 - not sure about mpl 2.x
...
class MyDonutPie(object):
    ...
    def plot(self, filename, size=1.0):
        ...
        # TODO: Enable rcParams below when moving to matplotlib >= 2.0
