...
def build_notifications_org_list(db):
    ...
    for request in db.RequestDoc.collection.find(
        {"_id": {"$in": ticket_owner_ids}}, {"_id": 1, "report_types": 1}
    ):
        if "CYHY" in request["report_types"]:
            ...
            logging.debug(
                "Added {} to notifications_to_generate".format(request["_id"])
            )
        ...
        logging.debug(
            "Checking for ancestors of {} with CYHY in their list of report_types".format(
                request["_id"]
            )
        )
...
def find_cyhy_parents(db, org_id):
    ...
    cyhy_parents = set()
    for request in db.RequestDoc.collection.find(
        {"children": org_id}, {"_id": 1, "report_types": 1}
    ):
        if "CYHY" in request["report_types"]:
...
def main():
    ...
    notifications_org_ids, notifications_not_generated = build_notifications_org_list(
        db
    )
    logging.debug(
        "Will attempt to generate notifications for {} orgs: {}".format(
            len(notifications_org_ids), notifications_org_ids
        )
    )

    # Create notification PDFs for CyHy orgs
    master_report_key = Config(args["CYHY_DB_SECTION"]).report_key
    num_pdfs_created = generate_notification_pdfs(
        db, notifications_org_ids, master_report_key
    )
    ...
    notifications_to_delete = sorted(
        set(notifications_not_generated) - set(orgs_notified)
    )
